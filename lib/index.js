...
var MasonScaffoldCommand = function (_Command) {
    ...
    _createClass(MasonScaffoldCommand, [{
        ...
        value: function prepareTemplate(template) {
            ...
            var templateValue = templates.get(template);
            var filename = void 0;
            ...
            var interactive = this.input.flags.indexOf('interactive') !== -1 || this.input.flags.indexOf('i') !== -1;
            var defaultValues = conf.definitions ? conf.definitions : {};
            var replacements = new Map(Object.entries(defaultValues));
            if (interactive) {
                (function () {
                    var prefix_match = (var_prefix + '').replace(/[\\"']/g, '\\$&').replace(/\u0000/g, '\\0');
                    var suffix_match = (var_suffix + '').replace(/[\\"']/g, '\\$&').replace(/\u0000/g, '\\0');
                    var match_txt = new RegExp(prefix_match + '([^ ]+)' + suffix_match, 'g');
                    var parts = buffer.match(match_txt);
                    var self = _this2;
                ...
                if (this.input.options) {
                    try {
                        ...
                        for (var varName in this.input.options) {
        ...
        value: function run(resolve, reject) {
            ...
            if (this.input.args.length) {
                (function () {
                    ...
                    ScaffoldUtil.registerConfigTemplates(_this3);

                    var template = _this3.input.args[0];
                    var destination = _this3.input.args[1];
                    if (!template || !destination) {
                        throw "Usage: mason scaffold [template] [destination] [--var=val]";
                    }

                    var templates = _this3.runner.data.get('scaffold.templates');
                    if (templates.has(template)) {
                        (function () {
                            var flags = _this3.input.flags.indexOf('f') !== -1 || _this3.input.flags.indexOf('force') !== -1 ? 'w+' : 'wx';
                            var withTemplate = function withTemplate(fd, template) {
                                // We have a valid write stream, parse the template
                                var buffer = _this3.prepareTemplate(template);
                            ...
                            var templateObj = templates.get(template);
                            var conf = ScaffoldUtil.getConf(_this3);
                            var destination_prefix = void 0;
                            ...
                            _fs2.default.open(destination, flags, function (err, fd) {
                                if (err) {
                                    if (err.code == "EEXIST") {
